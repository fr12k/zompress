...
pub fn compress(allocator: std.mem.Allocator, text: []const u8, config: CompressConfig) !CompressResult {
    ...
    defer {
        for (lines.items) |l| allocator.free(l.content);
    ...
    while (iter.next()) |raw| {
        ...
        try lines.append(allocator, .{ .content = duped, .is_blank = trimmed.len == 0 });
    }
    ...
    for (lines.items) |line| {
        if (line.is_blank) {
            ...
            try collapsed.append(allocator, line);
        } else {
            blank_run = 0;
            try collapsed.append(allocator, line);
        }
    ...
    for (collapsed.items, 0..) |line, i| {
        ...
        try deduped.append(allocator, line);
    }

    // 4. Truncate very long lines
    const max_line_len: usize = 500;
    for (deduped.items) |*line| {
        if (line.content.len > max_line_len) {
            const truncated = try std.fmt.allocPrint(allocator, "{s}... [{d} more chars]", .{
